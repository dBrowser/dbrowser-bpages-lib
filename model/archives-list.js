...
module.exports = class ArchivesList extends EventTarget {
  constructor ({listenNetwork} = {}) {
    ...
    // wire up events
    dbrowser.archives.addEventListener('added', this.onAdd.bind(this))
    dbrowser.archives.addEventListener('removed', this.onRemove.bind(this))
    dbrowser.archives.addEventListener('updated', this.onUpdate.bind(this))
    if (listenNetwork) {
      dbrowser.archives.addEventListener('network-changed', this.onNetworkChange.bind(this))
    }
  ...
  async setup (filter) {
    // fetch archives
    this.archives = await dbrowser.archives.list(filter)
  ...
  onAdd (e) {
    ...
    if (archive) return
    dbrowser.archives.get(e.details.url).then(archive => {
