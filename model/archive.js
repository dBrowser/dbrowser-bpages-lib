...
module.exports = class LibraryDatArchive extends DatArchive {
  constructor (url) {
    ...
    // wire up events
    dbrowser.archives.addEventListener('updated', (this.onLibraryUpdated = e => {
  ...
  destroy () {
    // unwire events
    this.listeners = {}
    dbrowser.archives.removeEventListener('updated', this.onLibraryUpdated)
  ...
  toggleSaved() {
    if (this.isSaved) {
      dbrowser.archives.remove(this.url).then(() => {
        this.info.userSettings.isSaved = false
        this.emitChanged()
      })
    } else {
      dbrowser.archives.add(this.url).then(() => {
